...
const DesktopImage:FC<{images: IImage[], hoveredId: number, catalogItemId: number}>= ({images, catalogItemId, item}) => {
    const {width} = useWindowSize()
    const areaRef = useRef<HTMLAnchorElement>(null)
    const [activeId, setActiveId] = useState(images && images[0]?.id)
    ...
    return (
        <Link href={`/products/${item.id}`} className={styles.wrapper} ref={areaRef}>
            <div className={styles.areas}>
                {images.map(image => (
                    <Area images={images} width={100/images.length} key={image.id} setActiveId={setActiveId} elementId={image.id} activeId={activeId}  catalogItemId={catalogItemId}/>
                ))}
            </div>
            {images.map(image => (
