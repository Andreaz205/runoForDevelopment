...
const Area:FC<{catalogItemId: number, hoveredId: number, activeId: number, width?: number, setActiveId: (id: number) => void, elementId: number}> = ({catalogItemId, width, setActiveId, elementId, activeId, images}) => {
    ...
    return (
        <div ref={ref} className={styles.area}>
            {/*<div className={cn({[styles.active]: activeId === elementId && hoveredId === catalogItemId}, {[styles.decoration]: hoveredId === catalogItemId && GridImages?.length > 1})}></div>*/}
            <div className={cn({[styles.active]: activeId === elementId})}></div>
        </div>
    );
};
